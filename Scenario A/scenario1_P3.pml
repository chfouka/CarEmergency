...
bool busyGarage = true; /*le busy all'inizio devono essere true senno' assert non si soddisfa.*/
bool busyTruck = true;
bool busyRental = true;

bool prenotatoGarage;
bool prenotatoTruck;
...
proctype Garage(  ){
 ...
 caremGarage ? request, x ->
                  ...
                  :: !busyGarage ->  prenotatoGarage = 1; caremGarage ! reply ( 1 );
                    atomic{ caremGarage ? release, _ ; prenotatoGarage = 0 }                               
                  fi
...
proctype Truck( ){
 ...
 caremTruck ? request, x ->
                  ...
                  :: !busyTruck ->  prenotatoTruck = 1; caremTruck ! reply ( 1 ); 
                    atomic { caremTruck ? release, _ ; prenotatoTruck = 0}
                  fi
...
/** Spin Safety verification: l'assert è verificata


spin -a  scenario1_P3.pml
...
State-vector 96 byte, depth reached 71, errors: 0
     2355 states, stored
     2463 states, matched
     4818 transitions (= stored+matched)
       12 atomic steps
hash conflicts:         0 (resolved)

Stats on memory usage (in Megabytes):
    0.243 equivalent memory usage for states (stored*(State-vector + overhead))
    0.378 actual memory usage for states
